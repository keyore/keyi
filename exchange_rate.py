...
def exchange_main():
    ...
    # currency_name_dict[currency_name]
    select_currency_code = currency_name_dict[currency_name]
    last_page = 10

    if clicked:
        # 환율코드 크롤링
        df_exchange = get_exchange_rate_data(currency_name_dict[currency_name], 10)
        ...
        df_exchange_rate.index = pd.to_datetime(df_exchange_rate_2.index)
